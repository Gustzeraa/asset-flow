...
export function LoginPage() {
  ...
  const { login } = useAuth()
  const EnterIcon = actionIcons.restore
  const SessionIcon = actionIcons.session
        ...
        >
          <section className="relative flex min-h-[52vh] items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_top_right,_rgba(59,130,246,0.08),_transparent_22%),linear-gradient(180deg,#ffffff_0%,#f4f8ff_100%)] px-5 py-7 sm:px-8 sm:py-8 md:px-10 md:py-10 lg:min-h-screen lg:px-10 lg:py-8 xl:px-12">
            ...
            <div className="relative z-10 flex w-full justify-center">
              <Box className="w-full max-w-[28rem] md:max-w-[30rem] lg:max-w-[28rem] xl:max-w-[30rem]" pos="relative">
                <LoadingOverlay
                  loaderProps={{ color: 'var(--mantine-color-brand-5)' }}
                  overlayProps={{ blur: 2, radius: 'xl' }}
                  visible={showEntryOverlay}
                  zIndex={1000}
                />
                <AppCard className="w-full border-slate-200/70 bg-white/94 p-4 shadow-[0_28px_70px_rgba(8,18,41,0.08)] sm:p-5">
                  <Stack gap="md">
