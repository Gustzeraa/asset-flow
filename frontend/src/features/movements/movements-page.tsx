...
export function MovementsPage() {
  ...
  const MovementsIcon = screenIcons.movements
  ...
  return (
    <Stack gap="lg">
      <PageHeader
        actions={
          <Group>
            <AppButton component="a" href={`/api/consumables/movements/export/${query ? `?${query}` : ''}`} leftSection={<DownloadIcon size={14} />} variant="light">
              Exportar Excel
            </AppButton>
          </Group>
        }
        description="Audite entradas e saídas com filtros refinados por período, responsável e destino."
        icon={<MovementsIcon size={18} />}
      ...
      <AppCard>
        <DataTable<Movement>
          ...
          emptyIcon={<MovementsIcon size={18} />}
